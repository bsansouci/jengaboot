...
/* Read the ppxFlag field in package.json */
let getPpxFlag libDir::libDir => {
  let packageJsonPath = rel dir::libDir "package.json";
  mapD
    (
      Dep.action_stdout (
    ...
    (
      fun content =>
        switch
          ...
          (List.hd (String.split content on::'\n' |> List.filter f::nonBlank)) {
        | None => None
        | Some content =>
          if (content == "") {
            None
        ...
        Dep.both
          ...
          (
            Dep.both
              ...
              (Dep.both (getThirdPartyOcamlfindLibs libDir::libDir) (getPpxFlag libDir::libDir))
          )
      )
      (
        fun (thirdPartyNpmLibs, (firstPartyDeps, (thirdPartyOcamlfindLibNames, ppxFlag))) => {
